...
var Student = require('../models/student');
 ...
 // Defines the JWTs contents, or payload, given a student object. Make
 // changes to what your token looks like here.
 function extractPayload(student, options) {
   return {
    _id: student._id,
    email: student.email,
    // name:  student.name,
 ...
 /**
  * Creates a JWT and returns it to the student in the response.
  ...
  * 2.  Status 403 Forbidden (when the student can't be find or password fails)
 ...
 function create(req, res, next) {
   ...
   Student
     ...
     .then(function(student) {
       if (!student || !student.verifyPasswordSync(req.body.password)) {
         var message = 'Student not found or password incorrect.';
         return res.status(403).json(message);
       }

       var token = generateJwt(student);
 ...
 /**
  * Creates a new JWT for an authenticated student (using the authenticate
 ...
 function generateJwt(student, options) {
   return jwt.sign(
     extractPayload(student, options),
